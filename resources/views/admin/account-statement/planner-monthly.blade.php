        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="page-title">
                    <h3>Account Statements - Planner Yearly Report</h3>
                </div>
            </div>
        </div>
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="row">
                                ...
                                @if(!auth()->user()->hasAnyRole(['Planner']))
                                    <div class="col-lg-4 mb-2">
                                        <div class="o-f-inp">
                                            <label>Planner</label>
                                ...
                                @else
                                    <div class="col-lg-4 mb-2">
                                        <div class="o-f-inp">
                                            <label>Planner</label>
                                            <select id="filter-planner" class="form-control shadow-none" readonly>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="report-header mb-4">
                        ...
                        <h3 class="text-center fw-bold mb-3">Planner Monthly Statement</h3>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function () {
            // Initialize Select2
            $('#filter-year').select2({ placeholder: "Select Year", allowClear: true });
            $('#filter-planner').select2({ placeholder: "Select Planner", allowClear: true });
            $('#filter-company').select2({ allowClear: true });
            ...
            function fetchPlannerMonthlyReport() {
                ...
                if (!year || !planner_id) {
                    $('#table tbody').html('<tr><td colspan="4" class="text-center text-muted">Please select a year & planner</td></tr>');
                    $('#table-footer').hide(); // Hide footer if no data
                    return;
                }

                $.ajax({
                    url: "{{ route('admin.planner.monthly-invoices') }}",
                    type: "GET",
                    data: { year, planner_id, company_id },
                    success: function (res) {
                        ...
                        for (let i = 1; i <= 12; i++) {
                            ...
                            // Note: stripping commas if your backend number_format adds them
                            let paid = monthData ? parseFloat(monthData.paid.replace(/,/g, '')) : 0;
                            let unpaid = monthData ? parseFloat(monthData.unpaid.replace(/,/g, '')) : 0;
                            ...
                            tableBody += `
                                                                                                                                                                                                                                                                                                        <tr>
                                                                                                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                                                                                                            <td class="clickable paid" data-month="${i}">${paid.toFixed(2)}</td>
                                                                                                                                                                                                                                                                                                            <td class="clickable unpaid" data-month="${i}">${unpaid.toFixed(2)}</td>
                        ...
                        $('#footer-paid').text(grandPaid.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
                        $('#footer-unpaid').text(grandUnpaid.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
                        $('#footer-total').text(grandTotal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
            ...
            $('#exportCsv').click(function () {
                ...
                if (!year || !planner) {
                    Swal.fire({
                        ...
                        text: 'Please select Year and Planner',
                ...
                let url =
                    "{{ route('admin.planner.monthly-export') }}"
                    + "?year=" + year
                    + "&planner_id=" + planner
                    + "&company_id=" + company;
            ...
            $('#exportPdf').click(function () {
                ...
                if (!year || !planner || !company) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Missing Filter',
                        text: 'Please select Year, Planner And Company',
                        confirmButtonText: 'OK'
                    });
                    return;
                }
                ...
                let url =
                    "{{ route('admin.planner.monthly-pdf') }}"
                    + "?year=" + year
                    + "&planner_id=" + planner
                    + "&company_id=" + company;
            ...
            $(document).on('click', '.clickable', function () {
                ...
                // Open modal
                $('#invoiceDetailsModalLabel').text(`${type.toUpperCase()} Invoices - ${monthNames[month - 1]}`);
                ...
                $('#invoiceDetailsBody').html('<tr><td colspan="7" class="text-center">Loading...</td></tr>');
                $('#invoiceDetailsModal').modal('show');

                // Fetch details via AJAX
                $.ajax({
                    url: "{{ route('admin.account-statements.monthly-invoices-details') }}",
                    type: 'GET',
                    data: { year, planner_id, month, type, company_id },
                    success: function (res) {
                        let body = '';
                        let grandTotal = 0;
                                ...
                                if (inv.payments && inv.payments.length > 0) {
                                    inv.payments.forEach(p => {
                                        ...
                                        body += `<tr>
                                                                                                                                                                                ...
                                                                                                                                                                                <td>${p.or || '-'}</td>
                                                                                                                                                                                <td class="text-end-cstm">${amt.toFixed(2)}</td>
                                                                                                                                                                                <td><span class="badge ${inv.type === 'paid' ? 'bg-success' : 'bg-warning'} text-capitalize">${inv.type}</span></td>
                                                                                                                                                                            </tr>`;
                                    });
                                }
                            });
