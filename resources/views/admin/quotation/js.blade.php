<script>
    $(function () {
        let table = $('#table').DataTable({
            ...
            ajax: {
                ...
                data: function (d) {
                    ...
                    d.quotation_date = $('#filter-quotation-date').val();
                    d.validity_date = $('#filter-validity-date').val();
                    d.status = $('#filter-status').val();
                    d.invoice = $('#filter-invoice').val();
                    d.type = $('#filter-type').val();
                    d.total_group = $('#filter-total-group').val();
                    d.work_plan = $('#filter-work-plan').val();
        ...
        $('#filter-status, #filter-quotation, #filter-customer, #filter-quotation-date, #filter-validity-date, #filter-invoice,#filter-type, #filter-total-group, #filter-work-plan, #filter-from-date, #filter-to-date')
        ...
        $('#reset-filters').on('click', function () {
            ...
            $('#filter-invoice').val('');
            $('#filter-customer').val('');
            $('#filter-quotation-date').val('');
            $('#filter-validity-date').val('');
            $('#filter-work-plan').val('');
        ...
        $(document).on('click', '.open-approval-modal', function () {
            ...
            $.get(`/admin/quotations/${quotationId}/approvals`, function (res) {
                let html = '';
                ...
                <div class="border rounded p-3 mb-3">
                    ...
                    <select name="approvals[${index}][status]" class="form-select approval-status" ${res.status === 'accepted' ? 'disabled' : ''}>
        ...
        $('#saveApproval').click(function (e) {
            ...
            $.post('/admin/quotations/approvals/update', $('#approvalForm').serialize(), function () {
        ...
        $(document).on('click', '.accept', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('admin.quotations.accept') }}",
                        type: "POST",
                        data: {
                            _token: "{{ csrf_token() }}",
        ...
        $(document).on('submit', '#commonForm', function (e) {
            ...
            let confirmMessage = status === 'approved' ?
                'This will approve the quotation' :
                'This will reject the quotation.';
    ...
    function deleteRow(id) {
        deleteRecord('/admin/quotations/' + id, 'table', 'Do you really want to delete this record?');
    }
</script>
<script>
