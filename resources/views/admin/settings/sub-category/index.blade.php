                    ...
                    <div class="row">
                        <form method="GET" id="searchForm">
                            <div class="row">
                                <div class="col-lg-3 mb-2">
                                    <div class="o-f-inp">
                                        ...
                                        <select name="category" id="category" class="form-select shadow-none">
                                            ...
                                            @foreach ($filterCategories as $record)
                                ...
                                <div class="col-lg-3 mb-2 d-flex align-items-end btn-w-cs">
                                    <button type="button" class="reset-filter-btn" id="resetBtn">Reset</button>
                                </div>
                                <div class="col-lg-3 d-flex align-items-end">
                                    <a href="{{ route('admin.sub.category.create') }}" class="add-btn">Add SubCategory</a>
                                </div>
                            </div>
                        </form>
                                ...
                                <div class="row justify-content-end">
                                    <div class="col-lg-7">
                                        <div class="entry-select">
                                            <p>Showing</p>
                                            <form method="GET" action="{{ route('admin.manage.sub.category') }}">
                                                ...
                                                <tr class="max-width-250">
                                                    ...
                                                    <td>
                                                        <div class="active-toggle">
                                                            <input type="checkbox" class="toggle-btn" data-id="{{ $cate->id }}"
                                                                {{ $cate->status ? 'checked' : '' }}
                                                                @cannot('business-subcategory.edit') disabled @endcannot>
                                                    ...
                                                    <td>
                                                        <div class="action-btns">
                                                            <a href="{{ route('admin.sub.category.edit', $cate->id) }}"
                                                            ...
                                                            <form id="delete-category-{{ $cate->id }}"
                                                                action="{{ route('admin.sub.category.destroy', $cate->id) }}"
...
@section('scripts')
    ...
    <script>
        $(document).on('change', '.toggle-btn', function () {
            ...
            $.ajax({
                url: "{{ route('admin.sub.category.toggle-status') }}",
                method: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    id: categoryId,
    ...
    <script>
        $(document).ready(function () {
            function fetchFilteredData() {
                ...
                $.ajax({
                    url: "{{ route('admin.manage.sub.category') }}",
                    type: "GET",
                    data: {
                        category: category,
                        status: status,
            ...
            $('#category,#status').on('keyup change', function () {
                fetchFilteredData();
            });
