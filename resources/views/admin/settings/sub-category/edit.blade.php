    ...
    <div class="row">
        <div class="col-lg-12 mb-3">
            <div class="main-table-container">
                <form action="{{route('admin.sub.category.update',$sub->id)}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        <div class="col-lg-4 mb-3 o-f-inp">
                            ...
                            <input type="text" name="code" class="form-control shadow-none" value="{{$sub->code}}" readonly>
                        </div>                       
                         <div class="col-lg-4 mb-3 o-f-inp">
                            <label for=""> Name <span class="text-danger">*</span>@error('name') <small class="text-danger">{{ $message }}</small> @enderror</label>
                            <input type="text" name="name" class="form-control shadow-none" value="{{$sub->name}}">
                        </div>
                        ...
                        <div class="col-lg-4 mb-3 o-f-inp">
                            ...
                            <select name="category" id="category" class="form-select shadow-none">
                                ...
                                <option value="{{$cate->id}}" {{ isset($sub) && $sub->category_id == $cate->id ? 'selected' : '' }}>{{$cate->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-lg-4 mb-3 o-f-inp">
                            ...
                            <select name="status" id="status" class="form-select shadow-none">
                                <option value="">--- Select ---</option>
                                <option value="1" {{ isset($sub) && $sub->status == 1 ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ isset($sub) && $sub->status == 0 ? 'selected' : '' }}>Inactive</option>
                            </select>
                        </div>
