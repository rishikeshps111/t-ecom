...
class StorePaymentRequest extends FormRequest
{
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            $invoice = Invoice::with('creditNotes')->find($invoiceId);

            if (!$invoice) {
                return;
            }

            $creditNoteTotal = $invoice->creditNotes->sum('amount');

            $finalBalance = max(0, $invoice->balance_amount - $creditNoteTotal);
